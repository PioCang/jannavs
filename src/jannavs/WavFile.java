...
public class WavFile
{
	...
	public File file;						// File that will be read from or written to
	public IOState ioState;				// Specifies the IO State of the Wav File (used for snaity checking)
	public int bytesPerSample;			// Number of bytes required to store a single sample
	public long numFrames;					// Number of frames within the data section
	public FileOutputStream oStream;	// Output stream used for writting data
	public FileInputStream iStream;		// Input stream used for reading data
	public double floatScale;				// Scaling factor used for int <-> float conversion
	public double floatOffset;			// Offset factor used for int <-> float conversion
	public boolean wordAlignAdjust;		// Specify if an extra byte at the end of the data chunk is required for word alignment
	...
	public int blockAlign;					// 2 bytes unsigned, 0x0001 (1) to 0xFFFF (65,535)
	public int validBits;					// 2 bytes unsigned, 0x0002 (2) to 0xFFFF (65,535)

	// Buffering
	public byte[] buffer;					// Local buffer used for IO
	public int bufferPointer;				// Points to the current position in local buffer
	public int bytesRead;					// Bytes read after last read into local buffer
	public long frameCounter;				// Current number of frames read or written
